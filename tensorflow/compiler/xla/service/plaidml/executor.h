...
// Declares the XlaPlaidMLExecutor class, which is a CPU-only implementation
// of the StreamExecutor interface. For now, this is used for testing and to
// examine the performance of host-based StreamExecutor code.
#ifndef TENSORFLOW_COMPILER_XLA_SERVICE_PLAIDML_EXECUTOR_H_
#define TENSORFLOW_COMPILER_XLA_SERVICE_PLAIDML_EXECUTOR_H_
...
class XlaPlaidMLExecutor : public internal::StreamExecutorInterface {
 public:
  explicit XlaPlaidMLExecutor(const PluginConfig &plugin_config);
  ~XlaPlaidMLExecutor() override;
  ...
  port::Status MemZero(Stream *stream, DeviceMemoryBase *location,
                       uint64 size) override {
    return port::InternalError("PlaidML can not memzero");
  }
  port::Status Memset(Stream *stream, DeviceMemoryBase *location, uint8 pattern,
                      uint64 size) override {
    return port::InternalError("PlaidML can not memset");
  }
...
#endif  // TENSORFLOW_COMPILER_XLA_SERVICE_PLAIDML_EXECUTOR_H_
